...
namespace SPW
{
		...
		unsigned int slot)
	{
		...
		std::string unkown_path = Config::k_WorkingProjectRoot;
		auto unkown_it = material.m_TextureIDMap.find(TextureMapType::Unknown);
		if (unkown_it != material.m_TextureIDMap.end())
		{
	...
	void bindPlights(const std::shared_ptr<Shader>& shader,
	                 const std::unordered_map<LightType, std::string>& light_inputs, const std::vector<PLight>& pLights)
	{
		if (light_inputs.find(LightType::PointLight) != light_inputs.end())
		{
	...
	inline void clearHelper(const std::shared_ptr<RenderPassNodeI>& node,
	                        const std::shared_ptr<RenderBackEndI>& backend)
	{
