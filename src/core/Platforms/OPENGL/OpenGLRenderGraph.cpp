...
namespace SPW {
    ...
    static glm::mat4 getCubeProjection() {
    ...
    void bindMaterial(
        ...
        std::string metallic_path = Config::k_WorkingProjectRoot;
        auto metallic_it = material.m_TextureIDMap.find(TextureMapType::Metalness);
        if (metallic_it != material.m_TextureIDMap.end()) {
            metallic_path += textures.at(metallic_it->second);
        }
        ...
        std::string unknown_path = Config::k_WorkingProjectRoot;
        auto unknown_it = material.m_TextureIDMap.find(TextureMapType::Unknown);
        if (unknown_it != material.m_TextureIDMap.end()) {
            unknown_path += textures.at(unknown_it->second);
        }
        ...
        for (auto &[type, name]: mat_inputs) {
            switch (type) {
                ...
                case MetallicType: {
                    ...
                    shader->setInt(name, slot);
                    backend->BindImageTex(metallic_path, slot);
                    slot++;
                    break;
                }
                case RoughnessType: {
    ...
    // bind lights
    void bindDlightMat(const std::shared_ptr<Shader> &shader, const glm::vec3 &cam_center, const std::string &name,
                       const DLight &light) {
    ...
    void bindDlights(const std::shared_ptr<Shader> &shader,
                     const std::unordered_map<LightType, std::string> &light_inputs,
    ...
    void bindDlight(const std::shared_ptr<Shader> &shader,
                    const std::unordered_map<LightType, std::string> &light_inputs,
    ...
    void bindPlight(const std::shared_ptr<Shader> &shader,
                    const std::unordered_map<LightType, std::string> &light_inputs, const PLight &light) {
    ...
    void bindPlights(const std::shared_ptr<Shader> &shader,
                     const std::unordered_map<LightType, std::string> &light_inputs,
                     const std::vector<PLight> &pLights) {
        if (light_inputs.find(LightType::PointLight) != light_inputs.end()) {
    ...
    inline void
    clearHelper(const std::shared_ptr<RenderPassNodeI> &node, const std::shared_ptr<RenderBackEndI> &backend) {
    ...
    void OpenGLRenderGraph::drawWithModels(const RenderInput &input,
                                           const std::unordered_map<UUID, std::vector<Entity>, UUID::hash> &
                                           render_models, RepeatType type, unsigned int repeat_id, int face) {
            ...
            for (auto &model: models) {
                ...
                if (face < 0) {
                    ...
                    if (type == RepeatForPLights) {
                        bindMVP(shader, shader_desc.transform_inputs, MM,
                                getCubeViewOnFace(input.pLights[repeat_id].position, face), getCubeProjection());
                    } else {
                        bindMVP(shader, shader_desc.transform_inputs, MM, getCubeViewOnFace(glm::vec3(0.f), face),
                                getCubeProjection());
                    }
                }
    ...
    void OpenGLRenderGraph::renderOnModelPassNode(const std::shared_ptr<ModelPassNode> &node, const ModelNodeMap &map,
    ...
    void OpenGLRenderGraph::renderOnModelRepeatPassNode(const std::shared_ptr<ModelRepeatPassNode> &node,
                                                        const ModelNodeMap &map, const RenderInput &input) {
        if (!node || node->ready) {
            return;
        }
        if (map.find(node->pass_id) == map.end()) {
    ...
    void OpenGLRenderGraph::renderOnModelToScreenNode(const std::shared_ptr<ModelToScreenNode> &node,
                                                      const ModelNodeMap &map, const RenderInput &input) {
        if (!node || node->ready) {
            return;
        }
    ...
    void OpenGLRenderGraph::renderOnScreenPassNode(const std::shared_ptr<ScreenPassNode> &node,
                                                   const RenderInput &input) {
        if (!node || node->ready) {
            return;
        }
    ...
    void
    OpenGLRenderGraph::renderOnImagePassNode(const std::shared_ptr<ImagePassNode> &node, const RenderInput &input) {
        if (!node || node->ready) {
            return;
        }
    ...
    void
    OpenGLRenderGraph::createResourceForImagePassNode(const std::shared_ptr<ImagePassNode> &node, unsigned int s_width,
