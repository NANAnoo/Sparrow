...
namespace SPW{
    ...
    auto const trigger_enter = [](SPW::Entity*) {


    };

    auto const trigger_leave = [](SPW::Entity*) {


    };

    auto const contact_enter = [](SPW::Entity*,SPW::Entity*) {


    };

    auto const contact_leave = [](SPW::Entity*,SPW::Entity*) {
            ...
            while(count--) {
                ...
                for (int i = 0; i < 2; ++i) {
                    ...
                    if(current.events & (PxPairFlag::eNOTIFY_TOUCH_FOUND)) {
                        contact_enter(gameObject,other_gameObject);

                    }

                    if(current.events & (PxPairFlag::eNOTIFY_TOUCH_LOST)) {
                        contact_leave(gameObject,other_gameObject);
        ...
        void onTrigger(physx::PxTriggerPair* pairs, physx::PxU32 count) override {
            ...
            while(count--)
            {
                ...
                if(current.status & PxPairFlag::eNOTIFY_TOUCH_FOUND) {
                    ...
                    trigger_enter(gameObject);

                }
                if(current.status & PxPairFlag::eNOTIFY_TOUCH_LOST){
                    PxShape* trigger_shape = pairs->triggerShape;
                    Entity* gameObject= static_cast<Entity *>(trigger_shape->userData);
                    trigger_leave(gameObject);
