...
#define IS_NEW_LINE(x) (((x) == '\r') || ((x) == '\n') || ((x) == '\0'))
    ...
    static inline bool parseReal(const char** token, float* out) {
        (*token) += strspn((*token), " \t");
        const char* end = (*token) + strcspn((*token), " \t\r");
        double val;
    ...
    static inline bool parseVertexWithColor(float* x, float* y, float* z,
    ...
    static void InitTexOpt(texture_option_t* texopt, const bool is_bump) {
        ...
        texopt->bump_multiplier = static_cast<float>(1.0);
        texopt->clamp = false;
        texopt->blendu = true;
        texopt->blendv = true;
        texopt->sharpness = static_cast<float>(1.0);
        texopt->brightness = static_cast<float>(0.0);
        texopt->contrast = static_cast<float>(1.0);
        texopt->origin_offset[0] = static_cast<float>(0.0);
        texopt->origin_offset[1] = static_cast<float>(0.0);
        texopt->origin_offset[2] = static_cast<float>(0.0);
        texopt->scale[0] = static_cast<float>(1.0);
        texopt->scale[1] = static_cast<float>(1.0);
        texopt->scale[2] = static_cast<float>(1.0);
        texopt->turbulence[0] = static_cast<float>(0.0);
        texopt->turbulence[1] = static_cast<float>(0.0);
        texopt->turbulence[2] = static_cast<float>(0.0);
    ...
    struct TinyObjPoint {
        ...
        TinyObjPoint(float x_, float y_, float z_) :
            x(x_), y(y_), z(z_) {}
    };
    ...
    inline float dot(const TinyObjPoint& v1, const TinyObjPoint& v2) {
        return (v1.x * v2.x + v1.y * v2.y + v1.z * v2.z);
    }

    inline float GetLength(TinyObjPoint& e) {
        return std::sqrt(e.x * e.x + e.y * e.y + e.z * e.z);
    }

    inline TinyObjPoint Normalize(TinyObjPoint e) {
        float inv_length = float(1) / GetLength(e);
    ...
    static bool exportGroupsToShape(shape_t* shape, const PrimGroup& prim_group,
        ...
        bool triangulate, const std::vector<float>& v,
        std::string* warn) {
            ...
            for (size_t i = 0; i < prim_group.faceGroup.size(); i++) {
                ...
                if (triangulate && npolys != 3) {
                    if (npolys == 4) {
                        ...
                        float length_n = GetLength(n);
                        //Check if zero length normal
                        if (length_n <= 0) {
                            continue;
                        }
                        //Negative is to flip the normal to the correct direction
                        float inv_length = -float(1.0) / length_n;
                        n.x *= inv_length;
                        n.y *= inv_length;
                        n.z *= inv_length;
                        ...
                        axis_w = n;
                        TinyObjPoint a;
                        if (std::abs(axis_w.x) > float(0.9999999)) {
                        ...
                        while (remainingFace.vertex_indices.size() > 3 &&
                            ...
                            float area = (vx[0] * vy[1] - vy[0] * vx[1]) * static_cast<float>(0.5);
        ...
        while (inStream->peek() != -1) {
            ...
            if (token[0] == 'T' && token[1] == 'r' && IS_SPACE(token[2])) {
                ...
                if (has_d) {
                    ...
                    material.dissolve = static_cast<float>(1.0) - parseReal(&token);
                }
                has_tr = true;
                continue;
            }
